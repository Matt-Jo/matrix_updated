<?php
/*
 $Id: ot_lev_discount.php,v 1.1 2002/08/30 21:05:04 wilt Exp $

 osCommerce, Open Source E-Commerce Solutions
 http://www.oscommerce.com

 Copyright (c) 2002 osCommerce

 Released under the GNU General Public License
*/

 if (!defined('MODULE_LEV_DISCOUNT_TITLE')) define('MODULE_LEV_DISCOUNT_TITLE', 'Price Break Discount');
 if (!defined('MODULE_LEV_DISCOUNT_DESCRIPTION')) define('MODULE_LEV_DISCOUNT_DESCRIPTION', 'Price Break Discount');
 if (!defined('SHIPPING_NOT_INCLUDED')) define('SHIPPING_NOT_INCLUDED', ' [Shipping not included]');
 if (!defined('TAX_NOT_INCLUDED')) define('TAX_NOT_INCLUDED', ' [Tax not included]');
?>
